#!/usr/bin/env python3
"""
Kubernetes Setup Scripts Validation Tool - Fixed Scripts Version
Validates k8s_master_node_fixed.sh and k8s_worker_node_fixed.sh for common issues
"""
...
def validate_script(script_path, script_type):
    ...
    print(f"\n--- Validating {script_type} Script (Fixed Version) ---")
    ...
    if content.startswith('#!/usr/bin/env bash') or content.startswith('#!/bin/bash'):
        print("✓ Shebang present and correct")
    else:
...
def main():
    ...
    master_script = Path(scripts_path) / "k8s_master_node_fixed.sh"
    worker_script = Path(scripts_path) / "k8s_worker_node_fixed.sh"
    
    print("=== Kubernetes Setup Scripts Validation (Fixed Versions) ===")
    
    # Check if scripts exist
    if not master_script.exists():
        print(f"ERROR: Fixed master script not found: {master_script}")
        sys.exit(1)
    
    if not worker_script.exists():
        print(f"ERROR: Fixed worker script not found: {worker_script}")
        sys.exit(1)
    
    print("✓ Fixed scripts found")
    ...
    if total_issues > 0:
        print("\n❌ SCRIPTS STILL HAVE CRITICAL ISSUES")
        sys.exit(1)
    elif total_warnings > 0:
        print("\n⚠️  SCRIPTS HAVE WARNINGS - REVIEW RECOMMENDED")
        print("✅ BUT CRITICAL ISSUES ARE FIXED!")
        sys.exit(0)
    else:
