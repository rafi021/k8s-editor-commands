...
function Test-ScriptContent {
    ...
    $unquotedVars = [regex]::Matches($content, '\$\{?[A-Z_]+\}?(?!["\w])')
    if ($unquotedVars.Count -gt 0 -and $Detailed) {
        $warnings += "Found potentially unquoted variables - review for proper quoting"
    }
